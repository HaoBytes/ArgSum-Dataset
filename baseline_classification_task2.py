...
"""
This script is used to finetune the Flan-T5-3B/11B for Evidence dataset Task 2 baseline.
"""
...
def load_dataset(input_file, input_val_file, input_test_file, val=True) -> pd.DataFrame:
    ...
    train_df["Topic_Argument"] = train_df["Topic"] + " " + train_df["Argument"]
    df_train = train_df[['Topic_Argument', 'Evidence_1', 'Evidence_2', 'Label']]
    df_train.columns = ["text_a", "text_b", "text_c", "labels"]
    # Convert labels 1 to 'True' and labels 10 to 'False'
    df_train['target_text'] = df_train['labels'].replace({1: 'True', 0.5: 'Neutral', 0: 'False'})
    
    val_df["Topic_Argument"] = val_df["Topic"] + " " + val_df["Argument"]
    df_val = val_df[['Topic_Argument', 'Evidence_1', 'Evidence_2', 'Label']]
    df_val.columns = ["text_a", "text_b", "text_c", "labels"]
    df_val['target_text'] = df_val['labels'].replace({1: 'True', 0.5: 'Neutral', 0: 'False'}) 

    test_df["Topic_Argument"] = test_df["Topic"] + " " + test_df["Argument"]
    df_test = test_df[['Topic_Argument', 'Evidence_1', 'Evidence_2', 'Label']]
    df_test.columns = ["text_a", "text_b", "text_c", "labels"]
    df_test['target_text'] = df_test['labels'].replace({1: 'True',0.5: 'Neutral',  0: 'False'}) 
...
def preprocess_function(examples):
    ...
    inputs = [prefix + text_a + " </s> " + text_b + " </s> " + text_c for text_a, text_b, text_c in zip(examples["text_a"], examples["text_b"], examples["text_c"])]
    model_inputs = tokenizer(inputs, add_special_tokens=True, max_length=max_input_length, truncation=True)
...
def compute_metrics(labels, preds):
    ...
    return accuracy, mcc, macro_f1, micro_f1
