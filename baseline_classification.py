...
"""
This script is used to finetune the Flan-T5-3B/11B for Evidence dataset Task 1 baseline.
"""
...
def load_dataset(input_file, input_val_file, input_test_file, val=True) -> pd.DataFrame:
    ...
    df_train = train_df[['argument', 'evidence', 'label']]
    df_train.columns = ["text_a", "text_b", "labels"]
    # Convert labels 1 to 'True' and labels 10 to 'False'
    df_train['target_text'] = df_train['labels'].replace({1: 'True', 0: 'False'})
    
    df_val = val_df[['argument', 'evidence', 'label']]
    df_val.columns = ["text_a", "text_b", "labels"]
    df_val['target_text'] = df_val['labels'].replace({1: 'True', 0: 'False'}) 

    df_test = test_df[['argument', 'evidence', 'label']]
    df_test.columns = ["text_a", "text_b", "labels"]
    df_test['target_text'] = df_test['labels'].replace({1: 'True', 0: 'False'}) 
...
def preprocess_function(examples):
    ...
    inputs = [prefix + text_a + " </s> " + text_b for text_a, text_b in zip(examples["text_a"], examples["text_b"])]
