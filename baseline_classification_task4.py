...
def load_dataset(input_file, input_val_file, input_test_file, val=True) -> pd.DataFrame:
    ...
    df_train = train_df[['input', 'concat_column', 'fact_checking']]
    df_train.columns = ["text_a", "text_b", "labels"]
    # Convert labels 1 to 'True' and labels 0 to 'False'
    df_train['target_text'] = df_train['labels']
    
    df_val = val_df[['input', 'concat_column', 'fact_checking']]
    df_val.columns = ["text_a", "text_b", "labels"]
    df_val['target_text'] = df_val['labels']

    df_test = test_df[['input', 'concat_column', 'fact_checking']]
    df_test.columns = ["text_a", "text_b", "labels"]
    df_test['target_text'] = df_test['labels']
